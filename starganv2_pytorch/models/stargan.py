...
class AdainResidualBlock(nn.Module):
    ...
    def _shortcut(self, x):
        if self.upsample:
            x = F_torch.interpolate(x, scale_factor=2, mode="nearest")
        if self.learned_sc:
            x = self.conv1x1(x)
        return x
...
class HighPass(nn.Module):
    ...
    def forward(self, x):
        ...
        return F_torch.conv2d(x, filter, padding=1, groups=x.size(1))
...
class Generator(nn.Module):
    def __init__(self, img_size=256, style_dim=64, max_conv_dim=512, w_hpf=1):
        ...
        for _ in range(repeat_num):
            ...
            self.decode.insert(
                0, AdainResidualBlock(dim_out, dim_in, style_dim,
        ...
        for _ in range(2):
            ...
            self.decode.insert(
                0, AdainResidualBlock(dim_out, dim_out, style_dim, w_hpf=w_hpf))
    ...
    def forward(self, x, s, masks=None):
        ...
        for block in self.decode:
            ...
            if (masks is not None) and (x.size(2) in [32, 64, 128]):
                ...
                mask = F_torch.interpolate(mask, size=x.size(2), mode="bilinear")
